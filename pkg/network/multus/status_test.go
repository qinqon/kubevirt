...
var _ = Describe("Network Status", func() {
	Context("SecondaryNetworkStatusesByPodIfaceName", func() {
		DescribeTable("It should return empty", func(networkStatuses []networkv1.NetworkStatus) {
			Expect(multus.SecondaryNetworkStatusesByPodIfaceName(networkStatuses)).To(BeEmpty())
		},
		...
		It("Should return a map of pod interface name to network status containing just secondary networks", func() {
			...
			result := multus.SecondaryNetworkStatusesByPodIfaceName(networkStatuses)
	...
	Context("NetworkStatusesFromPod", func() {
		DescribeTable("should return an empty slice", func(podAnnotations map[string]string) {
			...
			Entry("when network status annotation is missing",
				map[string]string{},
			),
			Entry("when network status annotation is empty",
				map[string]string{networkv1.NetworkStatusAnnot: ""},
			),
			Entry("when network status annotation is illegal",
				map[string]string{networkv1.NetworkStatusAnnot: "not a valid JSON array"},
			),
		)
	...
	Context("LookupPodPrimaryIfaceName", func() {
		...
		DescribeTable("Should return the primary pod interface name", func(networkStatuses []networkv1.NetworkStatus, expectedResult string) {
			...
			Entry("When the primary pod interface name is reported and its the custom value",
				[]networkv1.NetworkStatus{
