...
var _ = Describe("VMI network spec", func() {
	...
	Context("IndexPodIfacesNamesByNetworkName", func() {
		const (
		...
		It("Should return an empty map when there are no networks an no interface statuses", func() {
			res, err := netvmispec.IndexPodIfacesNamesByNetworkName([]v1.Network{}, []v1.VirtualMachineInstanceNetworkInterface{})
		...
		It("should return a map when there is a single element", func() {
			...
			res, err := netvmispec.IndexPodIfacesNamesByNetworkName(networks, interfacesStatus)
			Expect(err).NotTo(HaveOccurred())
